        ...
        while (not self.es_final()):
            ...
            if (not bool(jugadas_legales)):
                ...
                if (self.turno=='N'):
                    ...
                    aj, jugada = self.decision_maquina(aj,algoritmoN,profundidadN)
                    self.jugada(jugada,dado1,dado2)
                    self.imprimir_tablero() 
                else:
                    print(f"\nTurno de {algoritmoB} {profundidadB}")
                    aj, jugada = self.decision_maquina(aj,algoritmoB,profundidadB)
                    self.jugada(jugada,dado1,dado2)
                    self.imprimir_tablero()      
...
resultados1EE = [((('expectiminimax', 1), 2), (('expectiminimax', 2), 8)),
...
resultados1ElEl = [((('expectiminimax_limitado', 1), 7), (('expectiminimax_limitado', 2), 3)),
